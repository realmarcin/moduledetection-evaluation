...
class ModulesComparison():
    ...
    def score(self, baselines, scorenames = ["rr", "rp"]):
        ...
        if "rr" in scorenames:
            ...
            print("recovery {:.4f}\trelevance {:.4f}\tfF1rr {:.4f}".format(scores["recovery"], scores["relevance"], scores["F1rr"]))
...
def modevalworker(setting, scores, baseline):
    ...
    for regnet_name in dataset["knownmodules"].keys():
        ...
        for knownmodules_name in dataset["knownmodules"][regnet_name].keys():
            ...
            settingscores_goldstandard = modevalscorer(modules, knownmodules, regnet_name + "__" + knownmodules_name, setting['method_location'], baselineoi)
